...
def custom_dataset_page():
    ...
    <div class="page-title-block">
        ...
        <p>Upload your dataset and generate structured insights like the main dashboard.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        c3.metric("Missing Values", int(missing))

        # ─────────────────────────────
        # CATEGORICAL DISTRIBUTION (LIKE SENTIMENT BAR)
        # ─────────────────────────────
        if categorical_cols:
            st.markdown("### Category Distribution")

            cat_col = st.selectbox("Select column", categorical_cols)
            ...
            # Pie Chart (like sentiment share)
            with col2:
        ...
        # ─────────────────────────────
        # GROUP ANALYSIS (LIKE PRODUCT PERFORMANCE)
        # ─────────────────────────────
        if len(categorical_cols) >= 2:
            st.markdown("### Group Analysis")

            col_x = st.selectbox("Group by", categorical_cols, key="group1")
            col_y = st.selectbox("Compare with", categorical_cols, key="group2")
        ...
        # ─────────────────────────────
        # NUMERIC DISTRIBUTION (LIKE CONFIDENCE HIST)
        # ─────────────────────────────
        if numeric_cols:
            ...
            ax4.hist(df[num_col].dropna(), bins=25)
            ax4.set_title(f"{num_col} Distribution")
            st.pyplot(fig4)
        ...
        if len(numeric_cols) > 1:
            ...
            sns.heatmap(df[numeric_cols].corr(), annot=True, ax=ax5)
            st.pyplot(fig5)

        # ─────────────────────────────
        # TIME TREND (LIKE MONTHLY TREND)
        # ─────────────────────────────
        ...
        if possible_dates and numeric_cols:
            ...
            dcol = st.selectbox("Select date column", possible_dates)
            ncol = st.selectbox("Select value column", numeric_cols, key="trend")

            df[dcol] = pd.to_datetime(df[dcol], errors='coerce')
            trend = df.groupby(df[dcol].dt.to_period('M'))[ncol].mean()

            fig6, ax6 = plt.subplots()
            ax6.plot(trend.index.astype(str), trend.values, marker='o')
            plt.xticks(rotation=45)
            ax6.set_title(f"{ncol} Trend Over Time")
            st.pyplot(fig6)
# ─────────────────────────────────────────────
# SESSION ROUTING
# ─────────────────────────────────────────────
